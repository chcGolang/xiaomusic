...
lxapihost="http://192.168.43.147:1011"
lxapipass="NS4gNoV3UDFENh7ijP3Vmw=="
async def getmy_playlist(type="netease",api_host="http://192.168.43.68:13000", playlist_ids=[],uid=None,br=128,proxy="HTTP:%2F%2F192.168.43.68:8065"):
    """
    Purpose:
    """
        ...
        if uid:
            ...
            for item in music_list["playlist"]:
                ...
                for music in musics["playlist"]["tracks"]:
                    ...
                    name = music.get("name")
                    url = f"{api_host}/song/url?id={music['id']}&br={br}000&realIP=211.161.244.70&proxy={proxy}"
                ...
                for music in res['cdlist'][0]['songlist']:
                    ...
                    try:
                        ...
                        for song in response.json()['result']['songs']:
                            apiurl = f"{api_host}/song/url?id={song['id']}&br={br}000&proxy={proxy}"
                            songurl = ""
                            if song['originSongSimpleData']:
                               songurl = f"{api_host}/song/url?id={song['originSongSimpleData']['songId']}&br={br}000&proxy={proxy}"
                            ...
                            if song['originSongSimpleData'] and song['originSongSimpleData']['name'] == name and song['originSongSimpleData'][0]['artists'] == artist:
                                url = songurl
                                apiurl = songurl
                                lxurl = f"{api_host}/song/url?id={song['originSongSimpleData']['songId']}&br={br}000&proxy=HTTP:%2F%2F127.0.0.1:8080"
                                picUrl = song["al"]["picUrl"]
                                log.info(f"getmy_playlist api_url:{url}")
                                break

                            if song['originSongSimpleData'] and song['originSongSimpleData']['name'] == name:
                                url = songurl
                                apiurl = songurl
                                lxurl = ""
                                picUrl = song["al"]["picUrl"]
